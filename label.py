...
def mostrar_imagem(nome_imagem):
    ...
    global imagem_original, largura_original, altura_original, escala_x, escala_y
    imagem_original = pygame.image.load(path)
    largura_original, altura_original = imagem_original.get_size()
    escala_x = largura_original / SCREEN_SIZE[0]
    escala_y = altura_original / SCREEN_SIZE[1]
    img = pygame.transform.scale(imagem_original, SCREEN_SIZE)
    screen.blit(img, (0, 0))
...
def guardar_anotacoes(nome_imagem, pontos):
    ...
    with open(path, "w") as f:
        for linha in pontos:
            if linha:
                line_str = " ".join(f"{int(x * escala_x)} {int(y * escala_y)}" for (x, y) in linha)
                f.write(line_str + "\n")
            else:
                f.write("\n")
        ...
        with open(path, "r") as f:
            ...
            for idx, line in enumerate(lines):
                if idx < 3:
                    ...
                    pontos[idx] = [(int(values[i] / escala_x), int(values[i + 1] / escala_y))
...
# PRIMEIRO: mostrar imagem para definir escala
mostrar_imagem(imagens[indice])

# DEPOIS: carregar pontos
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Esquerdo → adicionar ponto
                linha = pontos[linha_atual]
                if len(linha) < 100:
                    linha.append(pos)

            elif event.button == 3:  # Direito → arrastar ponto
                dragging = False
                drag_index = None
                drag_linha = None
