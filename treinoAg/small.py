...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('training_lite.log')
    ]
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train Lite U-Net for image segmentation on Jetson Nano')
    ...
    parser.add_argument('--img_size', type=int, default=96, help='Image size (square)')
    parser.add_argument('--batch_size', type=int, default=16, help='Batch size')
    parser.add_argument('--epochs', type=int, default=100, help='Number of epochs')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate')
    parser.add_argument('--patience', type=int, default=5, help='Patience for early stopping')
    parser.add_argument('--seed', type=int, default=78942, help='Random seed')
    parser.add_argument('--output_dir', type=str, default='output_lite', help='Output directory')
...
# ==== DATASET - Simplified for speed ====
class LineDataset(Dataset):
    def __init__(self, image_paths, mask_paths, img_size=(64, 64), augment=False):
    ...
    def __getitem__(self, idx):
        ...
        # Simple augmentations (minimal processing)
        if self.augment and random.random() > 0.5:
            # Just horizontal flip (very fast)
...
class LiteSeparableConv(nn.Module):
...
class LiteConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Use regular conv for the first layer to maintain accuracy
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            # Use depthwise separable for the second layer for efficiency
            LiteSeparableConv(out_channels, out_channels)
        )
...
class MobileUNet(nn.Module):
    """Lightweight UNet with depthwise separable convolutions"""
    def __init__(self, in_channels=3, out_channels=1, features=[32, 64, 128, 256]):
        ...
        for feature in features:
            self.downs.append(LiteConvBlock(in_channels, feature))
            in_channels = feature
        
        # Up part - Decoder with skip connections
        for feature in reversed(features):
            # Transposed convolution to upsample
            self.ups.append(
                nn.ConvTranspose2d(feature*2, feature, kernel_size=2, stride=2)
            )
            # Double convolution after concat
            self.ups.append(LiteConvBlock(feature*2, feature))
        
        # Bottleneck
        self.bottleneck = LiteConvBlock(features[-1], features[-1]*2)
...
def train(args):
    ...
    logger.info(f"Starting lightweight model training at {start_time}")
...
def quantize_model(model, dataloader, device, args):
    ...
    try:
        ...
        quantized_model = torch.quantization.quantize_dynamic(
            model.to('cpu'),  # Move to CPU for quantization
            {nn.Conv2d, nn.ConvTranspose2d},  # Specify layers to quantize
            dtype=torch.qint8
        )
