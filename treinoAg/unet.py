...
class LaneFollowingSystem:
    def __init__(self):
        ...
        self.roi_height_ratio = 0.7  # Usar % inferior da imagem
        ...
        self.speed = 50  # km/h inicial (simulado)
...
# ==== PROGRAMA PRINCIPAL ====
def main():
    # Inicializar o sistema de seguimento de faixas
    ...
    # Configurar parâmetros de vídeo
    fps = cap.get(cv2.CAP_PROP_FPS)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    
    # Configurar gravação de vídeo
    out = cv2.VideoWriter('output_lane_following.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
    ...
    while cap.isOpened():
        ...
        # Processar o frame
        processed_frame, steering_angle = lane_system.process_frame(frame)
        
        # Exibir e gravar o resultado
        resized_frame = cv2.resize(processed_frame, (640, 480))
        cv2.imshow('Lane Following System', resized_frame)
