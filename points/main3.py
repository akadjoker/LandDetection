...
def inference_video_with_kalman_and_arrow(model_path, video_path, output_path=None, img_size=256):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    with torch.no_grad():
        while cap.isOpened():
            ...
            # === Cálculo do ângulo e steering ===
            angle_rad, steering = calculate_steering(lane_points)

            # === Desenhar seta ===
            draw_steering_arrow(frame, lane_points[3], lane_points[4], angle_rad)

            # Mostrar valor do steering no topo
            cv2.putText(frame, f"Steering: {steering:.2f}", (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

            cv2.imshow('Lane Detection with Kalman & Steering', frame)
...
if __name__ == "__main__":
    ...
    inference_video_with_kalman_and_arrow(
